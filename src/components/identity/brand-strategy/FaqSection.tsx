...
const faqs = [
  {
    q: 'Quanto dura un progetto di brand strategy?',
    a: 'Da 4 a 10 settimane, in base alla complessità e al numero di sessioni di workshop incluse. Un\'azienda con più linee di prodotto o sub-brand richiede più tempo.',
  },
  {
    q: 'È necessario partire dalla strategy prima di fare identità visiva?',
    a: "Sì, fortemente. L'identità visiva dovrebbe riflettere la strategia — non il contrario. Partire dal logo prima di avere un posizionamento chiaro porta spesso a dover rifare tutto.",
  },
  {
    q: 'Lavorate anche con aziende già avviate o solo con startup?',
    a: "Soprattutto con aziende già avviate. Il rebranding e il repositioning sono spesso più complessi (e più urgenti) di partire da zero.",
  },
  {
    q: 'Il documento finale è solo per uso interno?',
    a: 'No. È progettato per essere utilizzato da tutti i fornitori che lavoreranno con il brand: agenzie, designer, copywriter, sviluppatori.',
  },
  {
    q: "Collaborate con altri fornitori per l'implementazione?",
    a: 'Sì. Possiamo gestire noi l\'intera implementazione (identità visiva, comunicazione, digital) o fornire la strategia a partner selezionati dal cliente.',
  },
];
...
export default function FaqSection() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from('.ibst-faq-head', {
        opacity: 0, y: 24, duration: 0.9, ease: 'power3.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 80%' },
      });
      gsap.from('.ibst-faq-item', {
        opacity: 0, y: 20, duration: 0.8, stagger: 0.08, ease: 'power3.out',
        scrollTrigger: { trigger: '.ibst-faq-item', start: 'top 85%' },
      });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <section ref={sectionRef} className="w-full bg-[#F8F9FA] px-6 py-24 md:py-36">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="ibst-faq-head mb-14">
        ...
        <div className="divide-y divide-black/[0.07]">
          {faqs.map((faq, i) => (
            <div key={i} className="ibst-faq-item">
              <button
                onClick={() => toggle(i)}
