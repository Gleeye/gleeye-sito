...
const faqs = [
  {
    q: 'Possono includere anche le guidelines per i social?',
    a: 'Sì. Includiamo specifiche per i template social: dimensioni, griglie, stile fotografico, tono di voce per piattaforma.',
  },
  {
    q: 'Qual è il formato di consegna?',
    a: 'PDF interattivo con indice cliccabile, più file InDesign o Figma editabile. Il cliente può aggiornare il documento internamente o affidarci futuri aggiornamenti.',
  },
  {
    q: 'Le brand guidelines sono utili anche per le PMI?',
    a: 'Sì, soprattutto per le PMI che stanno crescendo. Più si cresce, più fornitori si coinvolgono — e più il rischio di incoerenza aumenta.',
  },
  {
    q: 'Quanto dura la produzione?',
    a: 'Da 3 a 6 settimane, in base alla complessità del brand e al numero di capitoli necessari.',
  },
  {
    q: 'Possono essere prodotte anche per un brand che non avete creato voi?',
    a: 'Sì. Spesso lavoriamo su brand esistenti — documentando e sistematizzando ciò che esiste già.',
  },
];
...
export default function FaqSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from('.ibg-faq-head', {
        opacity: 0, y: 24, duration: 0.9, ease: 'power3.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 80%' },
      });
      gsap.from('.ibg-faq-item', {
        opacity: 0, y: 20, duration: 0.8, stagger: 0.08, ease: 'power3.out',
        scrollTrigger: { trigger: '.ibg-faq-item', start: 'top 85%' },
      });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <section ref={sectionRef} className="w-full bg-[#F8F9FA] px-6 py-24 md:py-36">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="ibg-faq-head mb-14">
        ...
        <div className="divide-y divide-black/[0.07]">
          {faqs.map((faq, i) => (
            <div key={i} className="ibg-faq-item">
              <button
                onClick={() => toggle(i)}
