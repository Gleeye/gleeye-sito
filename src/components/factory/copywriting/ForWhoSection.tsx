...
const items = [
  {
    number: '01',
    text: 'Chi cerca le parole migliori per descrivere e promuovere la propria attività — senza dover inventarsi copywriter.',
  },
  {
    number: '02',
    text: 'Chi vuole ridare lustro al business con una comunicazione originale e brillante, fedele allo stile e ai valori dell\'azienda.',
  },
  {
    number: '03',
    text: 'Chi ha bisogno di testi per il sito, post social, script video o slogan per campagne online e offline — tutto coerente.',
  },
];

export default function CopywritingForWhoSection() {
  const sectionRef = useRef<HTMLElement>(null);

  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from('.forwho-title', {
        opacity: 0,
        y: 30,
        duration: 1,
        ease: 'power3.out',
  ...
  return (
    <section ref={sectionRef} className="w-full bg-black px-6 py-24 md:py-36">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="forwho-title mb-16">
        ...
        <div className="divide-y divide-white/[0.06]">
          {items.map((item) => (
            <div
              key={item.number}
              className="forwho-item flex items-start gap-8 md:gap-16 py-10 md:py-12 group"
            >
