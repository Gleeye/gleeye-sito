...
const items = [
  {
    number: '01',
    text: 'Chi ha sottovalutato l\'impatto delle immagini sulla percezione del proprio brand — e poi si è chiesto perché i competitor sembravano più professionali.',
  },
  {
    number: '02',
    text: 'Chi usa ancora foto stock che non raccontano nulla di specifico sulla propria azienda, perdendo ogni occasione di distinguersi.',
  },
  {
    number: '03',
    text: 'Chi ha bisogno di un portfolio fotografico coerente — da usare su sito, social, presentazioni e materiali offline — tutto con la stessa voce visiva.',
  },
];

export default function FotografiaForWhoSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.from('.ffw-item', {
        ...
        scrollTrigger: { trigger: '.ffw-item', start: 'top 85%' },
      });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="divide-y divide-white/[0.06]">
          {items.map((item) => (
            <div
              key={item.number}
              className="ffw-item flex items-start gap-8 md:gap-16 py-10 md:py-12 group"
            >
