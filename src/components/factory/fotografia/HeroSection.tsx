...
const WORDS = ['IMMAGINI,', 'NON', 'SOLO', 'FOTOGRAFIE'];

export default function FotografiaHeroSection() {
  ...
  useEffect(() => {
    ...
    gsap.to(orb2Ref.current, {
      x: -60, y: 50, duration: 13, repeat: -1, yoyo: true, ease: 'sine.inOut', delay: 3,
    });
    gsap.to(orb3Ref.current, {
      x: 40, y: 40, duration: 8, repeat: -1, yoyo: true, ease: 'sine.inOut', delay: 1.5,
    });
  ...
  return (
    ...
    >
      {/* Gradient orbs */}
      <div ref={orb1Ref} className="pointer-events-none absolute top-[10%] right-[8%] w-[40vw] h-[40vw] max-w-[600px] max-h-[600px] rounded-full blur-[120px] opacity-20" style={{ background: '#4e92d8' }} />
      <div ref={orb2Ref} className="pointer-events-none absolute top-[45%] left-[2%] w-[35vw] h-[35vw] max-w-[500px] max-h-[500px] rounded-full blur-[140px] opacity-20" style={{ background: '#614aa2' }} />
      <div ref={orb3Ref} className="pointer-events-none absolute bottom-[5%] right-[25%] w-[28vw] h-[28vw] max-w-[400px] max-h-[400px] rounded-full blur-[100px] opacity-15" style={{ background: '#4e92d8' }} />
      ...
      <div className="relative z-10 flex flex-col items-center justify-center flex-1 py-12 text-center">
          ...
          {WORDS.map((word, i) => (
            <div key={word} className="overflow-hidden">
              <span
                ...
                className={`block font-satoshi font-black tracking-tight leading-none text-[11vw] md:text-[8.5vw] lg:text-[7vw] xl:text-[6rem] ${
                  i === WORDS.length - 1
                    ? 'bg-gradient-to-r from-[#4e92d8] to-[#614aa2] bg-clip-text text-transparent'
                    : 'text-white'
                }`}
              >
                {word}
        ...
        <p ref={subRef} className="mt-8 max-w-xl font-jakarta font-medium text-lg md:text-xl text-white/30 leading-relaxed">
          Un&apos;immagine sbagliata dice le cose giuste nel modo peggiore.
        </p>
      </div>
      ...
      <div ref={ctaRef} className="relative z-10 flex items-center justify-center gap-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="font-jakarta text-sm text-white/20">Reportage · Studio · Editorial</span>
        </div>
      </div>
    </section>
  );
