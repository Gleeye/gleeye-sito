...
export default function EventsCtaSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from('.ecta-content', { opacity: 0, y: 40, duration: 1.2, ease: 'power3.out', scrollTrigger: { trigger: sectionRef.current, start: 'top 80%' } });
    }, sectionRef);
    return () => ctx.revert();
  }, []);

  return (
    <section id="contatti" ref={sectionRef} className="w-full bg-[#F8F9FA] px-6 py-24 md:py-40">
      <div className="max-w-5xl mx-auto ecta-content text-center">

        <span className="block font-satoshi text-[10px] font-bold uppercase tracking-[0.25em] text-black/30 mb-8">Inizia da qui</span>
        ...
        <h2 className="font-satoshi font-black tracking-tight uppercase text-4xl md:text-6xl lg:text-7xl text-black leading-tight mb-6">
          Stai organizzando<br className="hidden md:block" /> un evento?
        </h2>

        <p className="font-jakarta font-medium text-lg md:text-xl text-black/45 leading-relaxed mb-14 max-w-xl mx-auto">
          Raccontaci il progetto. Ti diciamo cosa possiamo fare e come.
        </p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          >
            <span className="font-satoshi text-[10px] font-bold uppercase tracking-[0.2em] text-black/60 group-hover:text-black transition-colors duration-300">010 09 54 533</span>
          </a>
        </div>
