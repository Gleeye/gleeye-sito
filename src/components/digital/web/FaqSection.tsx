...
const faqs = [
  {
    ...
    a: 'Dipende dalla complessità: da un sito vetrina essenziale a un e-commerce con integrazioni. Forniamo preventivi dettagliati dopo un brief iniziale.',
  },
  {
    q: 'Su quali tecnologie lavorate?',
    a: 'Next.js per progetti ad alte performance, WordPress per chi ha bisogno di un CMS flessibile, Shopify per e-commerce. Scegliamo lo stack in base alle esigenze, non alle abitudini.',
  },
  {
    q: 'Il sito sarà ottimizzato per mobile?',
    a: 'Sì, mobile-first by default. Il design parte dal mobile e scala verso desktop, non il contrario.',
  },
  {
    q: 'Quanto dura un progetto?',
    a: 'Un sito vetrina standard richiede 4-6 settimane. Progetti più complessi (e-commerce, web app) anche 8-12 settimane.',
  },
  {
    q: 'Dopo il lancio offrite assistenza?',
    a: 'Sì. Includiamo un anno di supporto tecnico base. Per assistenza continuativa offriamo pacchetti di manutenzione mensile.',
  },
];

export default function WebFaqSection() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from('.dweb-faq-head', {
        opacity: 0, y: 24, duration: 0.9, ease: 'power3.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 80%' },
      });
      gsap.from('.dweb-faq-item', {
        opacity: 0, y: 20, duration: 0.8, stagger: 0.08, ease: 'power3.out',
        scrollTrigger: { trigger: '.dweb-faq-item', start: 'top 85%' },
      });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <section ref={sectionRef} className="w-full bg-[#F8F9FA] px-6 py-24 md:py-36">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="dweb-faq-head mb-14">
        ...
        <div className="divide-y divide-black/[0.07]">
          {faqs.map((faq, i) => (
            <div key={i} className="dweb-faq-item">
              <button
                onClick={() => toggle(i)}
