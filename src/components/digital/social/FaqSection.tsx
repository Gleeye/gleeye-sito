...
const faqs = [
  {
    q: 'Gestite tutti i social o si può scegliere?',
    a: 'Si sceglie insieme. Non ha senso essere su ogni piattaforma. Partiamo dai canali dove il tuo pubblico è davvero presente.',
  },
  {
    q: 'Con quale frequenza pubblicate?',
    a: 'Dipende dalla strategia: da 3 a 7 post settimanali per piattaforma. Più importante della frequenza è la coerenza.',
  },
  {
    q: 'Chi scrive i testi?',
    a: 'Il nostro team, in italiano. Ogni copy passa per una revisione strategica prima della pubblicazione.',
  },
  {
    q: 'Come misurate i risultati?',
    a: 'Report mensile con metriche selezionate in base agli obiettivi: awareness, engagement, lead generation, traffico al sito. Niente dashboard piene di numeri inutili.',
  },
  {
    q: 'Lavorate anche solo sulla strategia senza gestione?',
    a: 'Sì. Offriamo anche sessioni di consulenza strategica e costruzione del piano editoriale, che il cliente poi esegue autonomamente o con il suo team.',
  },
];

export default function SocialFaqSection() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.from('.dsoc-faq-head', {
        opacity: 0, y: 24, duration: 0.9, ease: 'power3.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 80%' },
      });
      gsap.from('.dsoc-faq-item', {
        opacity: 0, y: 20, duration: 0.8, stagger: 0.08, ease: 'power3.out',
        scrollTrigger: { trigger: '.dsoc-faq-item', start: 'top 85%' },
      });
    }, sectionRef);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <section ref={sectionRef} className="w-full bg-[#F8F9FA] px-6 py-24 md:py-36">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="dsoc-faq-head mb-14">
        ...
        <div className="divide-y divide-black/[0.07]">
          {faqs.map((faq, i) => (
            <div key={i} className="dsoc-faq-item">
              <button
                onClick={() => toggle(i)}
