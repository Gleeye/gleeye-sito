...
const stats = [
  { value: '50+',  label: 'Brand gestiti sui social',        sub: 'tra PMI, startup e aziende strutturate' },
  { value: '3M+',  label: 'Impression generate',             sub: 'negli ultimi 12 mesi tra tutti i clienti gestiti' },
  { value: '8+',   label: 'Anni di attività',                sub: 'su piattaforme che nel frattempo sono cambiate tre volte' },
  { value: '0',    label: 'Post generici',                   sub: 'ogni contenuto nasce da una strategia specifica' },
];
  ...
  return (
    <section ref={sectionRef} className="w-full bg-[#F8F9FA] px-6 py-20 md:py-28 border-b border-black/[0.05]">
      <div className="max-w-7xl mx-auto grid grid-cols-2 lg:grid-cols-4 gap-px bg-black/[0.05]">
        {stats.map((s) => (
          <div key={s.value} className="dsoc-stat-item bg-[#F8F9FA] px-8 py-10 md:px-12 md:py-14 flex flex-col gap-3">
            <span
