...
export default function SeoServicesSection() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      gsap.from('.dseo-svc-header', {
        opacity: 0, y: 20, duration: 0.9, ease: 'power3.out',
        scrollTrigger: { trigger: sectionRef.current, start: 'top 80%' },
      });

      gsap.utils.toArray<HTMLElement>('.dseo-svc-row').forEach((row, i) => {
        gsap.from(row, {
          opacity: 0, y: 30, duration: 0.9, delay: i * 0.07, ease: 'power3.out',
          scrollTrigger: { trigger: row, start: 'top 88%' },
        });

        const line = row.querySelector('.dseo-svc-progress') as HTMLElement;
  ...
  return (
    <section ref={sectionRef} className="w-full bg-black px-6 py-24 md:py-36">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="dseo-svc-header flex items-end justify-between gap-6 mb-4">
        ...
        <div className="mt-12">
          {services.map((s) => (
            <div
              key={s.number}
              className="dseo-svc-row group relative border-t border-white/[0.07] py-8 md:py-10 cursor-default overflow-hidden"
            >
              ...
              <div
                className="dseo-svc-progress absolute bottom-0 left-0 h-px origin-left"
                style={{
                  width: '100%',
                  transform: 'scaleX(0)',
